...
func TestCheckForkActivation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			block:               eth.L2BlockRef{Time: timeint.FromUint64SecToMilli(10), Number: 5, Hash: common.Hash{0x5}},
			expectedCurrentFork: Regolith,
			expectedLog:         "Detected hardfork activation block",
		},
		{
			name:                "Still Regolith",
			block:               eth.L2BlockRef{Time: timeint.FromUint64SecToMilli(11), Number: 6, Hash: common.Hash{0x6}},
			expectedCurrentFork: Regolith,
			expectedLog:         "",
		},
		{
			name:                "Canyon activation",
			block:               eth.L2BlockRef{Time: timeint.FromUint64SecToMilli(20), Number: 7, Hash: common.Hash{0x7}},
			expectedCurrentFork: Canyon,
			expectedLog:         "Detected hardfork activation block",
		},
		{
			name:                "No more hardforks",
			block:               eth.L2BlockRef{Time: timeint.FromUint64SecToMilli(700), Number: 8, Hash: common.Hash{0x8}},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			chainSpec.CheckForkActivation(lgr, eth.L2BlockRef{Time: tt.block.Time - timeint.FromUint64SecToMilli(1), Number: 1, Hash: common.Hash{0x1}})
