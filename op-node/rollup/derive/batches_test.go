...
var zeroSeconds = timeint.FromUint64SecToSec(0)
...
func TestValidBatch(t *testing.T) {
	...
	l1BLate := eth.L1BlockRef{
		...
		Number:     l1A.Number + 1,
		ParentHash: l1A.Hash,
		Time:       l2A4.Time.ToSeconds() + timeint.FromUint64SecToSec(1), // too late for l2A4 to adopt yet
	}
		...
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{
					...
					EpochNum:     rollup.Epoch(l2A1.L1Origin.Number),
					EpochHash:    l2A1.L1Origin.Hash,
					Timestamp:    l2A1.Time.ToSeconds() + timeint.FromUint64SecToSec(1), // 1 too high
					Transactions: nil,
				},
			},
		...
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{
					...
					EpochNum:     rollup.Epoch(l2A1.L1Origin.Number),
					EpochHash:    l2A1.L1Origin.Hash,
					Timestamp:    l2A0.Time.ToSeconds(), // repeating the same time
					Transactions: nil,
				},
			},
			Expected: BatchDrop,
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{
					...
					EpochNum:     rollup.Epoch(l2A1.L1Origin.Number),
					EpochHash:    l2A1.L1Origin.Hash,
					Timestamp:    l2A1.Time.ToSeconds() - timeint.FromUint64SecToSec(1), // block time is 2, so this is 1 too low
					Transactions: nil,
				},
			},
			Expected: BatchDrop,
		...
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{
					...
					Timestamp:    (l2B0.Time + defaultBlockTime).ToSeconds(), // pass the timestamp check to get too epoch check
					Transactions: nil,
				},
			},
			Expected: BatchDrop,
		...
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{
					...
					Timestamp:    l2Y0.Time.ToSeconds(), // valid, but more than 6 ahead of l1Y.Time
					Transactions: []hexutil.Bytes{[]byte("sequencer should not include this tx")},
				},
			},
			Expected: BatchDrop,
		...
		},
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: &SingularBatch{ // we build l2B0', which starts a new epoch too early
					...
					EpochNum:     rollup.Epoch(l2B0.L1Origin.Number),
					EpochHash:    l2B0.L1Origin.Hash,
					Timestamp:    (l2A2.Time + defaultBlockTime).ToSeconds(),
					Transactions: nil,
				},
			},
			Expected: BatchDrop,
	...
	spanBatchTestCases := []ValidBatchTestCase{
		{
			...
			Batch: BatchWithL1InclusionBlock{
				...
				}, timeint.FromUint64SecToSec(0), big.NewInt(0)),
			},
			Expected:    BatchUndecided,
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: initializedSpanBatch([]*SingularBatch{
					...
					},
					{
						...
						Timestamp:    (l2B0.Time + defaultBlockTime*2).ToSeconds(),
						Transactions: nil,
					},
				}, timeint.FromUint64SecToSec(0), big.NewInt(0)),
			},
			...
			Batch: BatchWithL1InclusionBlock{
				...
				Batch: initializedSpanBatch([]*SingularBatch{
					...
					},
					{
						...
						Timestamp:    l2Z0.Time.ToSeconds(), // valid, but more than 6 ahead of l1Y.Time
						Transactions: nil,
					},
				}, timeint.FromUint64SecToSec(0), big.NewInt(0)),
			},
				...
				Batch: initializedSpanBatch([]*SingularBatch{
					{
						...
						EpochNum:     rollup.Epoch(l2A1.L1Origin.Number),
						EpochHash:    l2A1.L1Origin.Hash,
						Timestamp:    l2A0.Time.ToSeconds() + 1,
						Transactions: nil,
					},
					{
						ParentHash:   l2A1.Hash,
						EpochNum:     rollup.Epoch(l2A2.L1Origin.Number),
						EpochHash:    l2A2.L1Origin.Hash,
						Timestamp:    l2A1.Time.ToSeconds() + 1,
						Transactions: nil,
					},
				}, timeint.FromUint64SecToSec(0), big.NewInt(0)),
			},
