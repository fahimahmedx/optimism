...
func (cr *ChannelInReader) WriteChannel(data []byte) error {
	if f, err := BatchReader(bytes.NewBuffer(data), cr.spec.MaxRLPBytesPerChannel(cr.prev.Origin().Time), cr.cfg.IsFjord(cr.prev.Origin().Time*1000)); err == nil {
		cr.nextBatchFn = f
...
func (cr *ChannelInReader) NextBatch(ctx context.Context) (Batch, error) {
	...
	case SingularBatchType:
		...
		return batch, nil
	case SpanBatchType:
		if origin := cr.Origin(); !cr.cfg.IsDelta(origin.Time * 1000) {
