...
func checkSingularBatch(cfg *rollup.Config, log log.Logger, l1Blocks []eth.L1BlockRef, l2SafeHead eth.L2BlockRef, batch *SingularBatch, l1InclusionBlock eth.L1BlockRef) BatchValidity {
	...
	if batch.Timestamp < batchOrigin.Time.ToMilliseconds() {
	...
	if max := (batchOrigin.Time + spec.MaxSequencerDrift(batchOrigin.Time)).ToMilliseconds(); batch.Timestamp > max {
		if len(batch.Transactions) == 0 {
			...
			if epoch.Number == batchOrigin.Number {
				...
				if batch.Timestamp >= nextOrigin.Time.ToMilliseconds() { // check if the next L1 origin could have been adopted
...
func checkSpanBatch(ctx context.Context, cfg *rollup.Config, log log.Logger, l1Blocks []eth.L1BlockRef, l2SafeHead eth.L2BlockRef,
	...
	if batch.GetTimestamp() < nextTimestamp {
		...
		parentNum = l2SafeHead.Number - ((l2SafeHead.Time - batch.GetTimestamp()) / cfg.BlockTime).ToUint64Milli() - 1
		var err error
	...
	for i := 0; i < batch.GetBlockCount(); i++ {
		...
		if blockTimestamp < l1Origin.Time.ToMilliseconds() {
		...
		spec := rollup.NewChainSpec(cfg)
		// Check if we ran out of sequencer time drift
		if max := l1Origin.Time + spec.MaxSequencerDrift(l1Origin.Time); blockTimestamp > max.ToMilliseconds() {
			if len(batch.GetBlockTransactions(i)) == 0 {
				...
				if !originAdvanced {
					...
					if blockTimestamp >= l1Blocks[originIdx+1].Time.ToMilliseconds() { // check if the next L1 origin could have been adopted
						log.Info("batch exceeded sequencer time drift without adopting next origin, and next L1 origin would have been valid")
						return BatchDrop
					} else {
						log.Info("continuing with empty batch before late L1 block to preserve L2 time invariant")
