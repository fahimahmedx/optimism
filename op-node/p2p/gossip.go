...
func (p *publisher) PublishL2Payload(ctx context.Context, envelope *eth.ExecutionPayloadEnvelope, signer Signer) error {
	...
	if p.cfg.IsEcotone(timeint.FromUint64SecToSec(uint64(envelope.ExecutionPayload.Timestamp))) {
		return p.blocksV3.topic.Publish(ctx, out)
	} else if p.cfg.IsCanyon(timeint.FromUint64SecToSec(uint64(envelope.ExecutionPayload.Timestamp))) {
