...
func NewL2Genesis(config *DeployConfig, block *types.Block) (*core.Genesis, error) {
	...
	optimismChainConfig := params.ChainConfig{
		...
		RegolithTime:                  config.RegolithTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		CanyonTime:                    config.CanyonTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		ShanghaiTime:                  config.CanyonTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		CancunTime:                    config.EcotoneTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		EcotoneTime:                   config.EcotoneTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		FjordTime:                     config.FjordTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
		InteropTime:                   config.InteropTime(timeint.Seconds(block.Time())).FromSecPtrToUint64Ptr(),
