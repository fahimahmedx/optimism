...
func NewOpGeth(t testing.TB, ctx context.Context, cfg *SystemConfig) (*OpGeth, error) {
	...
	rollupGenesis := rollup.Genesis{
		...
		L2Time:       timeint.Seconds(l2GenesisBlock.Time()),
	...
	genesisPayload, err := eth.BlockAsPayload(l2GenesisBlock, cfg.DeployConfig.CanyonTime(timeint.Seconds(l2GenesisBlock.Time())))
...
func (d *OpGeth) AddL2Block(ctx context.Context, txs ...*types.Transaction) (*eth.ExecutionPayloadEnvelope, error) {
	...
	envelope, err := d.l2Engine.GetPayload(ctx, eth.PayloadInfo{ID: *res.PayloadID, Timestamp: timeint.FromUint64ToSec(uint64(attrs.Timestamp))})
...
func (d *OpGeth) CreatePayloadAttributes(txs ...*types.Transaction) (*eth.PayloadAttributes, error) {
	...
	l1Info, err := derive.L1InfoDepositBytes(d.l2Engine.RollupConfig(), d.SystemConfig, d.sequenceNum, d.L1Head, timeint.Seconds(timestamp))
	if err != nil {
		return nil, err
	}
