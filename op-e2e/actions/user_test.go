...
func runCrossLayerUserTest(gt *testing.T, test hardforkScheduledTest) {
	...
	if test.canyonTime != nil {
		require.Zero(t, (uint64(*test.canyonTime)*uint64(time.Second))%(dp.DeployConfig.L2BlockTime*uint64(time.Millisecond)), "canyon fork must be aligned")
	}
	if test.ecotoneTime != nil {
		require.Zero(t, (uint64(*test.ecotoneTime)*uint64(time.Second))%(dp.DeployConfig.L2BlockTime*uint64(time.Millisecond)), "ecotone fork must be aligned")
	}
