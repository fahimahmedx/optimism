...
func TestHardforkMiddleOfSpanBatch(gt *testing.T) {
	...
	require.NoError(t, err)
	require.False(t, sd.RollupCfg.IsDelta(timeint.FromUint64SecToSec(unsafeHeader.Time).ToMilliseconds()))
	...
	unsafeHeader, err = minerCl.HeaderByNumber(t.Ctx(), unsafeOriginNum)
	require.NoError(t, err)
	require.True(t, sd.RollupCfg.IsDelta(timeint.FromUint64SecToSec(unsafeHeader.Time).ToMilliseconds()))
