...
func TestL2Sequencer_SequencerDrift(gt *testing.T) {
	...
	for sequencer.SyncStatus().UnsafeL2.Time*uint64(time.Second)+sd.RollupCfg.BlockTime*uint64(time.Millisecond) < origin.Time*uint64(time.Second) {
	...
	for sequencer.SyncStatus().UnsafeL2.Time*uint64(time.Second)+sd.RollupCfg.BlockTime*uint64(time.Millisecond) <= (origin.Time+sd.ChainSpec.MaxSequencerDrift(origin.Time))*uint64(time.Second) {
