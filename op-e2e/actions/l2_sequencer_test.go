...
func TestL2Sequencer_SequencerDrift(gt *testing.T) {
	...
	for sequencer.SyncStatus().UnsafeL2.Time+sd.RollupCfg.BlockTime < timeint.FromUint64SecToMilli(origin.Time) {
	...
	for sequencer.SyncStatus().UnsafeL2.Time+sd.RollupCfg.BlockTime <= timeint.FromUint64SecToMilli(origin.Time)+sd.ChainSpec.MaxSequencerDrift(timeint.FromUint64SecToSec(origin.Time)).ToMilliseconds() {
