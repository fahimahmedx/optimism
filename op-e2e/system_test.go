...
func TestConfirmationDepth(t *testing.T) {
	...
	seqInfo, err := derive.L1BlockInfoFromBytes(sys.RollupConfig, timeint.Seconds(l2SeqHead.Time()), l2SeqHead.Transactions()[0].Data())
	require.NoError(t, err)
	...
	verInfo, err := derive.L1BlockInfoFromBytes(sys.RollupConfig, timeint.Seconds(l2VerHead.Time()), l2VerHead.Transactions()[0].Data())
	require.NoError(t, err)
...
func TestL1InfoContract(t *testing.T) {
	...
	fillInfoLists := func(start *types.Block, contract *bindings.L1Block, client *ethclient.Client) ([]*derive.L1BlockInfo, []*derive.L1BlockInfo) {
		...
		for b := start; ; {
			...
			infoFromTx, err := derive.L1BlockInfoFromBytes(sys.RollupConfig, timeint.Seconds(b.Time()), b.Transactions()[0].Data())
			require.NoError(t, err)
			txList = append(txList, infoFromTx)

			ecotone := sys.RollupConfig.IsEcotone(timeint.Seconds(b.Time())) && !sys.RollupConfig.IsEcotoneActivationBlock(timeint.Seconds(b.Time()))
	...
	for h := maxL1Hash; ; {
		...
		l1blocks[h] = &derive.L1BlockInfo{
			Number:         b.NumberU64(),
			Time:           timeint.Seconds(b.Time()),
			BaseFee:        b.BaseFee(),
			BlockHash:      h,
			SequenceNumber: 0, // ignored, will be overwritten
			BatcherAddr:    sys.RollupConfig.Genesis.SystemConfig.BatcherAddr,
		}
		if sys.RollupConfig.IsEcotone(timeint.Seconds(b.Time())) && !sys.RollupConfig.IsEcotoneActivationBlock(timeint.Seconds(b.Time())) {
...
func testFees(t *testing.T, cfg SystemConfig) {
	...
	if !sys.RollupConfig.IsEcotone(timeint.Seconds(sys.L2GenesisCfg.Timestamp)) {
	...
	require.NoError(t, err)
	require.Equal(t, sys.RollupConfig.IsEcotone(timeint.Seconds(header.Time)), gpoEcotone, "GPO and chain must have same ecotone view")

	gpoFjord, err := gpoContract.IsFjord(nil)
	require.NoError(t, err)
	require.Equal(t, sys.RollupConfig.IsFjord(timeint.Seconds(header.Time)), gpoFjord, "GPO and chain must have same fjord view")

	gpoL1Fee, err := gpoContract.GetL1Fee(&bind.CallOpts{}, bytes)
	require.Nil(t, err)

	adjustedGPOFee := gpoL1Fee
	if sys.RollupConfig.IsFjord(timeint.Seconds(header.Time)) {
	...
	} else if sys.RollupConfig.IsRegolith(timeint.Seconds(header.Time)) {
	...
	if !sys.RollupConfig.IsEcotone(timeint.Seconds(header.Time)) { // FeeScalar receipt attribute is removed as of Ecotone
		require.Equal(t,
			new(big.Float).Mul(
