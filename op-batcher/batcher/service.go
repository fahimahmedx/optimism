...
func (bs *BatcherService) initChannelConfig(cfg *CLIConfig) error {
	...
	if bs.UseBlobs && !bs.RollupConfig.IsEcotone(timeint.FromUint64SecToMilli(uint64(time.Now().Unix()))) {
	...
	if bs.ChannelConfig.CompressorConfig.CompressionAlgo.IsBrotli() && !bs.RollupConfig.IsFjord(timeint.FromUint64SecToMilli(uint64(time.Now().Unix()))) {
