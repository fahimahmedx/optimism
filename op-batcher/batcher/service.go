...
func (bs *BatcherService) initChannelConfig(cfg *CLIConfig) error {
	...
	if bs.UseBlobs && !bs.RollupConfig.IsEcotone(timeint.FromUint64SecToSec(uint64(time.Now().Unix()))) {
	...
	if bs.ChannelConfig.CompressorConfig.CompressionAlgo.IsBrotli() && !bs.RollupConfig.IsFjord(timeint.FromUint64SecToSec(uint64(time.Now().Unix()))) {
